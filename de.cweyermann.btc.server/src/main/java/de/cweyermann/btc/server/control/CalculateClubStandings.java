...
public class CalculateClubStandings {
    ...
    private void add(ClubStandings standings, Map<String, ClubStanding> club2Standing) {
        club2Standing.values()
                .stream()
                .filter(s -> s.getPoints() > 0.1)
                .forEach(s -> standings.addStandings(s));
    }
    ...
    private int getLastRound(Group group) {
        int max = group.getMatches()
                .stream()
                .max((a, b) -> a.getRoundnr() - b.getRoundnr())
                .get()
                .getRoundnr();
        return max;
    }
    ...
    private List<Match> getRound(Group group, int max) {
        List<Match> finals = group.getMatches()
                .stream()
